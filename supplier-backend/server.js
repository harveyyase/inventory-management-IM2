...
app.post('/api/products', async (req, res) => {
    ...
    const productId = 'PRD' + Date.now(); // Optional: auto-generate product ID
    ...
    try {
        ...
        console.error('❌ POST /products failed:', err.message);
        res.status(500).json({ error: 'Database error during product insert' });
    }
});
...
app.delete('/api/products/:id', async (req, res) => {
    ...
    try {
        ...
        const result = await request.query('DELETE FROM Product WHERE productId = @productId');
        ...
        console.error('❌ DELETE /products/:id failed:', err.message);
        res.status(500).json({ error: 'Database error' });
    }
});
...
const connectionString = Driver={ODBC Driver 18 for SQL Server};Server=tcp:im2-server.database.windows.net,1433;Database=IM2;Uid=rigene;Pwd=r1geneh@rvey;Encrypt=yes;TrustServerCertificate=no;Connection Timeout=30;
